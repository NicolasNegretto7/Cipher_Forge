...
| **HU25** | Como usuario nuevo, quiero registrarme proporcionando mi nombre completo, correo electrónico, contraseña de forma obligatoria y teléfono opcional, aceptando los Términos y Condiciones & Política de Privacidad. | 3 | **Aceptada** | Formulario con validaciones en frontend y backend (DTOs/Validators), con casilla obligatoria de términos. |
...
| **HU20** | Como fotógrafo, quiero que se bloquee cualquier intento de acceso directo mediante URL a mis colecciones privadas por parte de usuarios no autorizados, para mantener la privacidad del contenido. | 3 | **Aceptada** | Bloqueo por backend (`403 Forbidden`) al intentar consultar endpoints de colecciones privadas sin token o sin asignación de acceso. |
| **HU21** | Como usuario nuevo, quiero recibir un código de verificación en mi correo electrónico, para asegurar que la casilla registrada me pertenece. | 3 | **Aceptada** | Registro y validación de código numérico temporal con expiración (`codigo_expiracion`), actualizando `email_verificado = TRUE`. |
...
| **HU19** | Como usuario nuevo, quiero que se impida el registro de usuarios duplicados utilizando un mismo correo electrónico ya existente, para contar con una única cuenta por correo. | 3 | **Aceptada** | Restricción de unicidad en base de datos con respuesta HTTP 409 Conflict y mensaje descriptivo en interfaz. |
...
| **HU10** | Como cliente, quiero descargar directamente imágenes o videos individuales en dos niveles de calidad ("Buena Calidad" estándar o "Alta Calidad" original). | 1 | **Aceptada** | Botones de descarga directa individual sin esperas: "Buena Calidad" (reescalada a 1280px limpia) y "Alta Calidad" (archivo original íntegro). |
...
2. **Descarga Directa en Dos Calidades:** Desde la vista de cliente autorizado, se presionó "Buena Calidad" sobre una imagen, descargándose en el navegador la versión limpia de 1280px en menos de un segundo; posteriormente se presionó "Alta Calidad", descargándose el archivo original de 12 MB sin marcas de agua.
...
| ID | Historia de Usuario | Puntos | Estado | Observaciones / Demostración |
| :--- | :--- | :--- | :--- | :--- |
| **HU16** | Como fotógrafo, quiero que se controle mi límite de almacenamiento (3 GB), impidiendo subidas si se supera la cuota y, en subidas múltiples, completando los archivos válidos y notificando únicamente los que excedan la cuota restante. | 5 | **Aceptada** | Cálculo acumulativo en backend con `SUM(tamanio)`. En subidas múltiples, guarda los archivos que caben e informa en la respuesta cuáles superaron el límite. |
| **HU28** | Como fotógrafo, quiero que los videos subidos (por mí o por invitados) se gestionen como clips o recortes con un límite máximo de 800 MB por video original subido, para optimizar el almacenamiento y no exponer el material completo. | 5 | **Aceptada** | Validador `MultimediaValidator` comprueba tamaño de binario y tipo MIME (`video/mp4`, `video/quicktime`), rechazando archivos mayores a 800 MB. |
...
| **HU32** | Como usuario, quiero que los videos subidos generen automáticamente un recorte de 15 segundos para la vista previa en la colección y conserven el video completo para la descarga directa, para garantizar una visualización ligera y reservar el archivo original. | 3 | **Aceptada** | `MediaProcessor.php` invoca FFmpeg en contenedor Docker (`-t 15 -preset veryfast`), guardando clip ligero para la galería y original completo en `/uploads/originals/`. |
...
1. **Objetivo del Sprint 5:** Completar la edición de datos básicos de archivos (HU22), perfil profesional de fotógrafo (HU18), favoritos de clientes (HU23), respaldos diarios automáticos con rotación de 3 copias (HU13) y entrega de guía de usuario con capacitación final (HU15).
...
| **Participantes:** | Lemuel Swec (Cliente / Sponsor), Nicolás Negretto (Líder / SM), Iván Sandoval (Sublíder / Backend), Augusto Fernández (Frontend) |
| **Objetivo del Sprint:** | Finalizar el 100% del Product Backlog: permitir la edición de títulos y descripciones de recursos multimedia, habilitar la edición de perfil del fotógrafo, implementar la lista privada de favoritos para clientes, programar los respaldos automáticos diarios de base de datos con rotación de las últimas 3 copias, y entregar la guía de uso con capacitación completa al sponsor. |
...
| **HU18** | Como fotógrafo, quiero editar mi información de perfil, para tener siempre mis datos actualizados. | 3 | **Aceptada** | Pantalla de perfil con nombre y teléfono, visible en el directorio público de fotógrafos (`GET /fotografos`). |
| **HU23** | Como usuario, quiero marcar como favorita una colección pública completa, para tener una lista de favoritos privada. | 3 | **Aceptada** | Toggle interactivo de favoritos persistido en tabla `favoritos`, accesible desde la vista privada de usuario. |
| **HU13** | Como fotógrafo, quiero que se realice un respaldo automático diario de la base de datos y se roten las últimas 3 copias, para mitigar el riesgo de pérdida de datos. | 5 | **Aceptada** | Volcado DDL y DML vía PDO (`SHOW CREATE TABLE` + `SELECT` con `quote()`), rotación FIFO eliminando el respaldo más antiguo cuando hay 3 o más, y registro en tabla `backups`. |
...
### Demostración del Incremento Funcional
1. **Perfil Profesional:** Se editó el perfil de Lemuel Swec actualizando nombre y teléfono. Se comprobó que el perfil figura disponible en el directorio de fotógrafos.
